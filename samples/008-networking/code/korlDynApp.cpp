...
GAME_INITIALIZE(gameInitialize)
{
	*g_gs = {};// clear all GameState memory before initializing the template
	...
	serverInitialize(
		&g_gs->serverState, 1.f/60, g_gs->kgtGameState.hKalPermanent, 
		g_gs->kgtGameState.hKalTransient, kmath::megabytes(5), 
		kmath::megabytes(5), GAME_NET_SERVER_LISTEN_PORT);
...
GAME_UPDATE_AND_DRAW(gameUpdateAndDraw)
{
	...
	if(ImGui::Begin("TESTING SERVER EXAMPLE"))
	{
		...
		if(kgtNetClientIsDisconnected(&g_gs->kNetClient))
		{
			...
			ImGui::InputText(
				"net address", g_gs->clientAddressBuffer, 
	...
	kgtNetClientStep(
		&g_gs->kNetClient, deltaSeconds, 0.1f*deltaSeconds, 
