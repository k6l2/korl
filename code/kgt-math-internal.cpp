	...
	size_t vertexColorOffset)
{
	...
	const size_t requiredVertexBytes = sizeof(v3f32) + sizeof(RgbaF32);
	korlAssert(vertexByteStride >= requiredVertexBytes);
	korlAssert(vertexPositionOffset <= vertexByteStride - sizeof(v3f32));
	korlAssert(vertexColorOffset    <= vertexByteStride - sizeof(RgbaF32));
	korlAssert(requiredVertexBytes*requiredVertexCount <= vertexDataBytes);
	...
	for(size_t t = 0; t < arrlenu(epaState->tris); t++)
	{
		...
		const RgbaF32 triColor = (t == epaState->nearestTriToOriginIndex
			? krb::GREEN : krb::WHITE);
		for(size_t e = 0; e < 3; e++)
		{
			*V3F32_STRIDE(o_vertexPositions, vertexByteStride, 3*t + e) = 
				epaState->vertexPositions[tri.vertexPositionIndices[e]];
			*RGBAF32_STRIDE(o_vertexColors, vertexByteStride, 3*t + e) = 
				triColor;
		}
	}
	return kmath::safeTruncateU16(requiredVertexCount);
