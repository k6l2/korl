...
LRESULT WINAPI SnapHelperWinProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:
        {
            SIZEMOVEDATA* sizemove = reinterpret_cast<SIZEMOVEDATA*>(LSMGet());
            HDC hDC = GetDC(hwnd);
...
LRESULT PrepareSizeMove(HWND hwnd, WPARAM action, DWORD dwPos)
{
    ...
    */
    ReleaseCapture();
    sm = reinterpret_cast<SIZEMOVEDATA*>(LSMGet());
    if(!sm)
    {
        WNDCLASSA wndcls;
        HINSTANCE hInstance = GetModuleHandleA(NULL);
        sm = reinterpret_cast<SIZEMOVEDATA*>(
			LocalAlloc(0, sizeof(SIZEMOVEDATA)));
        if(!sm)
        {
            /* error */
            return 1;
        ...
        case WM_SYSCOMMAND:
        {
            switch(wParam & 0xfff0)
            {
                ...
                case SC_SIZE:
                    ...
                    return 
						PrepareSizeMove(
							hwnd, wParam, static_cast<DWORD>(lParam));
                default:
                    break;
            }
            break;
...
BOOL CALLBACK EnumWindowsShakeMinimize(HWND hwnd, LPARAM lParam)
{
    ...
    if(hwnd != sizemove->hwnd && IsWindowVisible(hwnd) && !IsIconic(hwnd))
    {
        ...
        */
        ...
        pNode = reinterpret_cast<SHAKERESTORENODE*>(
			LocalAlloc(0, sizeof(SHAKERESTORENODE)));
        if(!pNode)
            return FALSE;
...
void LSMCleanup()
{
    SIZEMOVEDATA *sm = reinterpret_cast<SIZEMOVEDATA*>(LSMGet());
    if(sm)
    {
