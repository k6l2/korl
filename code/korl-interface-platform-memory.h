...
{
    void**                                     userAddressPointer;// address of a definitive pointer to the allocation the user wishes to defragment; the `parent` assumes full responsibility of the lifetime of this allocation
    ...
    void*                                      parent;// optional; pointer to the allocation which contains the memory holding *userAddressPointer; if this allocation exists in the same allocator we are defragmenting, then it _must_ be an element of the same Korl_Heap_DefragmentPointer array that this DefragmentPointer belongs to (failure to do so will result in undefined behavior; _very_ likely will cause a crash), otherwise it is considered an immutable allocation during defragmentation; some allocations will themselves contain pointers to other allocations; when a DefragmentPointer's userAddress is changed, all recursive children of it should be considered "stale"/dangling, since at that point the algorithm wont be able to update the pointer within the data struct that just moved if its child is also defragmented; the defragmentation algorithm needs to know this relationship so that it doesn't accidentally move a stale DefragmentPointer; also, note that this is not const, because a custom `onAllocationMoved` callback for a DefragmentPointer has the authority to modify the *parent as they see fit
